...
- (void)viewWillAppear:(BOOL)animated
{
	...
	[labelState setText:self.selectedTable.state];
}
...
{
// MainNetworkingDataSource *networkingDataSource = [[XPModel sharedInstance] mainNetworkingDataSource];
// [networkingDataSource checkIfReservedForPlaceWithId:self.selectedTable.place_id andTableNumber:self.selectedTable.table_id
// withCompletitionBlock:^(NSArray *items, NSError *error, NSDictionary *userInfo) {
// NSLog(@"Finished check table");
// if (items)
// {
// if (items.count > 0)
